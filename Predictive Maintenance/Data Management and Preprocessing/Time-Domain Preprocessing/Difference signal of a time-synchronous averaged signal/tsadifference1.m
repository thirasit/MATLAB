...
% To obtain the signal containing just the meshing components for gears 5 and 6, filter out the components of the shaft rotation, gears 1 and 2 and, 3 and 4 by specifying their gear ratios of 17 and 51 in orderList. 
% The signal components corresponding to the shaft rotation (order = 1) is always implicitly included in the computation.

figure
imshow("FindAndVisualizeTheDifferenceSignalOfACompoundTSASignalExample_01.png")
...
% For this example, generate TSA signal X, which is the simulated data from the vibration sensor mounted on the motor.
...
% The sine wave has a frequency of 32 Hz. 
% To filter out the unmodulated sine wave and the sidebands of the phase modulating signal, use 32 as the orderList.
...
% In this example, sineWaveRectangularPulse.mat contains the data of a sine wave modulated by a rectangular pulse. 
% X is a vector with the modulated sine wave data obtained at a shaft speed of 60 RPM. 
% The unmodulated sine wave has a frequency of 32 Hz and amplitude of 1.0 units.

% Load the data, and plot the difference signal of the modulated TSA signal X. 
% To obtain the difference signal, filter out the unmodulated sine wave and the sidebands of the modulation signal by specifying the frequency of 32 Hz in orderList. 
