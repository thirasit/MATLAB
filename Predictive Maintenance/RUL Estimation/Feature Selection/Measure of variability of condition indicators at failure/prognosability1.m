...
% In this example, consider the lifetime data of 10 identical machines with the following 6 potential condition indicators−constant, linear, quadratic, cubic, logarithmic, and periodic. 
...
% From the resultant table of prognosability values, observe that the linear, cubic, and logarithmic features have values closer to 1. 
...
% prognosabilityEnsemble.zip is a collection of 4 files where every file contains a timetable of lifetime data for each machine - tbl1.mat, tbl2.mat, tbl3.mat and tbl4.mat. 
...
figure
imshow("VisualizePrognosabilityOfLifetimeDataExample_01.png")
...
% Visualize the prognosability of the potential condition indicators with 'Machine' as the member variable and group the lifetime data by 'FaultCode'. 
% Grouping the lifetime data ensures that prognosability calculates the metric for each fault code separately.
prognosability(ens,'MemberVariable','Machine','GroupBy','FaultCode');

% prognosability returns a histogram plot with the features ranked by their prognosability values. 
% A higher prognosability value indicates a more suitable condition indicator. 
% For instance, the candidate feature Current has the highest degree of prognosability for machines with FaultCode 1.
