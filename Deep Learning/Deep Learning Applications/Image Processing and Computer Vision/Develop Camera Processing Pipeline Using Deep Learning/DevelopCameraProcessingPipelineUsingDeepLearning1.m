...
% To download the data set, go to https://data.vision.ee.ethz.ch/ihnatova/public/zr2d/Zurich-RAW-to-DSLR-Dataset.zip.
...
% To simulate the minimal traditional processing pipeline, demosaic the RGGB Bayer pattern of the RAW data using the demosaic (Image Processing Toolbox) function.
...
% The minibatchqueue object is useful for managing the mini-batching of observations in custom training loops.
...
% The next function of minibatchqueue yields the next mini-batch of data.
...
% To get a pretrained VGG-16 network, install vgg16.
...
% The MAE loss penalises the L^1 distance between samples of the network predictions and samples of the target image.
% L^1 is often a better choice than L^2 for image processing applications because it can help reduce blurring artifacts [4].
...
% Iterate through the test set and calculate the MSSIM and PSNR for each test image using the multissim (Image Processing Toolbox) and psnr (Image Processing Toolbox) functions.
