...
% Prepare the training and validation data by following the instructions in Preprocess Multiresolution Images for Training Classification Network (Image Processing Toolbox).
...
% The inceptionv3 function returns a pretrained Inception-v3 network.
...
% For more information, see Train Deep Learning Network to Classify New Images.
...
% Specify the hyperparameter settings for RMSProp by using the trainingOptions function.
...
% Then, create an array of blockedImage (Image Processing Toolbox) objects that manage the test images.
...
% For more information, see Preprocess Multiresolution Images for Training Classification Network (Image Processing Toolbox).
normalMaskLevel = 8;
testDir = fullfile(dataDir,"testing");
...
figure
imshow("ClassifyTumorsUsingMultiresolutionBlockedImagesExample_01.png")
axis off;
...
% Classify the blocks in each test image and calculate the confusion matrix using the apply (Image Processing Toolbox) function with the processing operations defined by the computeBlockConfusionMatrixForCamelyon16 helper function.
...
% The AUC for this data set is close to 1. You can use the AUC to fine-tune the training process.
figure
stairs(fpr,tpr,"-");
