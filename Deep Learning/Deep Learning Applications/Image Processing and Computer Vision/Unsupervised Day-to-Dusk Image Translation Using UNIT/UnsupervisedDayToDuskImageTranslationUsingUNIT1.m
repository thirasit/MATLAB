...
% For more information about UNIT, see Get Started with GANs for Image-to-Image Translation (Image Processing Toolbox).

%%% Download Data Set
% This example uses the CamVid data set [2] from the University of Cambridge for training.
% This data set is a collection of 701 images containing street-level views obtained while driving.

% Specify dataDir as the desired location of the data.
% Download the CamVid data set using the helper function downloadCamVidImageData.
...
% Create a UNIT generator network using the unitGenerator (Image Processing Toolbox) function.
...
% Create two discriminator networks, one for each of the source and target domains, using the patchGANDiscriminator (Image Processing Toolbox) function.
% Day is the source domain and dusk is the target domain.
...
%%% Batch Training Data
% Create a minibatchqueue object that manages the mini-batching of observations in a custom training loop.
% The minibatchqueue object also casts data to a dlarray object that enables automatic differentiation in deep learning applications.
...
% - Evaluate the model gradients using the dlfeval function and the modelGradientDisc and modelGradientGen helper functions.
...
% Create a dlarray object that inputs data to the generator.
% If a supported GPU is available for computation, then perform inference on a GPU by converting the data to a gpuArray object.
...
% The final layer of the generator network produces activations in the range [-1, 1].
% For display, rescale the activations to the range [0, 1].
% Also, rescale the input day image before display.
...
% For display, rescale the activations to the range [0, 1].
% Also, rescale the input dusk image before display.
