...
% Ordinary linear regression can be used to fit a straight line, or any function that is linear in its parameters, to data with normally distributed errors.
...
% First, assumption of linearity in the parameters is relaxed, by introducing the link function.
% Second, error distributions other than the normal can be modeled

%%% Generalized Linear Models
% A regression model defines the distribution of a response variable (often generically denoted as y) in terms of one or more predictor variables (often denoted x1, x2, etc.).
% The most commonly used regression model, the ordinary linear regression, models y as a normal random variable, whose mean is linear function of the predictors, b0 + b1*x1 + ... , and whose variance is constant.
% In the simplest case of a single predictor x, the model can be represented as a straight line with Gaussian distributions about each point.
figure
mu = @(x) -1.9+.23*x;
x = 5:.1:15;
yhat = mu(x);
...
% In a generalized linear model, the mean of the response is modeled as a monotonic nonlinear transformation of a linear function of the predictors, g(b0 + b1*x1 + ...) .
% The inverse of the transformation g is known as the "link" function.
% Examples include the logit (sigmoid) link and the log link.
% Also, y may have a non-normal distribution, such as the binomial or Poisson.
% For example, a Poisson regression with log link and a single predictor x can be represented as an exponential curve with Poisson distributions about each point.
figure
mu = @(x) exp(-1.9+.23*x);
x = 5:.1:15;
yhat = mu(x);
...
% This example involves an experiment to help model the proportion of cars of various weights that fail a mileage test.
% The data include observations of weight, number of cars tested, and number failed.
...
% This graph is a plot of the proportion of cars failing, as a function of weight.
...
% But how exactly should P depend on weight?
...
% However, this fit still has similar problems.
% The graph shows that the fitted proportion starts to decrease as weight goes above 4000; in fact it will become negative for larger weight values.
% And of course, the assumption of a normal distribution is still violated.

% Instead, a better approach is to use glmfit to fit a logistic regression model.
...
% First, it uses a fitting method that is appropriate for the binomial distribution.
% Second, the logistic link limits the predicted proportions to the range [0,1].

% For logistic regression, we specify the predictor matrix, and a matrix with one column containing the failure counts, and one column containing the number tested.
...
%%% Model Diagnostics
% The glmfit function provides a number of outputs for examining the fit and testing the model.
% For example, we can compare the deviance values for two models to determine if a squared term would improve the fit significantly.
[logitCoef2,dev2] = glmfit([weight weight.^2],[failed tested],'binomial','logit');
pval = 1 - chi2cdf(dev-dev2,1)

% The large p-value indicates that, for these data, a quadratic term does not improve the fit significantly.
% A plot of the two fits shows there is little difference in the fits.
figure
...
% To check the goodness of fit, we can also look at a probability plot of the Pearson residuals.
...
% (Without this standardization, the residuals would have different variances.)
figure
...
% Once we are satisfied with the model, we can use it to make predictions, including computing confidence bounds.
% Here we predict the expected number of cars, out of 100 tested, that would fail the mileage test at each of four weights.
figure
...
% For each of the five distributions that glmfit supports, there is a canonical (default) link function.
% For the binomial distribution, the canonical link is the logit.
% However, there are also three other links that are sensible for binomial models.
% All four maintain the mean response in the interval [0, 1].
figure
eta = -5:.1:5;
