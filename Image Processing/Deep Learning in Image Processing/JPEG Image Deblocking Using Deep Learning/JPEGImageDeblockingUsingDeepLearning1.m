...
% If you do not want to download the training data or train the network, then you can load the pretrained DnCNN network by typing load("trainedJPEGDnCNN.mat") at the command line.
...
% Create the randomPatchExtractionDatastore from the two image datastores.
% Specify a patch size of 50-by-50 pixels.
% Each image generates 128 random patches of size 50-by-50 pixels. Specify a mini-batch size of 128.
patchSize = 50;
patchesPerImage = 128;
...
%%% Set up DnCNN Layers
% Create the layers of the built-in DnCNN network by using the dnCNNLayers function.
...
% Train the DnCNN network using the trainNetwork (Deep Learning Toolbox) function.
...
% Convert the JPEG-compressed images from the RGB color space to the YCbCr color space using the rgb2ycbcr function.
I10ycbcr = rgb2ycbcr(I10);
I20ycbcr = rgb2ycbcr(I20);
I50ycbcr = rgb2ycbcr(I50);

%%% Apply the DnCNN Network
% In order to perform the forward pass of the network, use the denoiseImage function.
...
% Quantify the quality of the deblocked images through four metrics.
% You can use the jpegDeblockingMetrics helper function to compute these metrics for compressed and deblocked images at the quality factors 10, 20, and 50.
% This function is attached to the example as a supporting file.
