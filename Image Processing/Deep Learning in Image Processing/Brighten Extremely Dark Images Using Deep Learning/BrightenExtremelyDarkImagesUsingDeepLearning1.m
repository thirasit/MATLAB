...
% - Create an imageDatastore that reads the short exposure RAW images using a custom read function. The read function reads a RAW image using the rawread function, then separates the RAW Bayer pattern into separate channels for each of the four sensors using the raw2planar function. Normalize the data to the range [0, 1] by transforming the imageDatastore object.
% - Create an imageDatastore object that reads long-exposure RAW images and converts the data to an RGB image in one step using the raw2rgb function. Normalize the data to the range [0, 1] by transforming the imageDatastore object.
...
% The example creates the encoder and decoder subnetworks using the blockedNetwork function.
...
% Combine the encoder subnetwork, bridge layers, decoder subnetwork, and final layers using the encoderDecoderNetwork function.
...
% Define the overall loss using the custom layer ssimLossLayerGray.
% This layer definition is attached to this example as a supporting file.
% The ssimLossLayerGray layer uses a loss of the form
% lossOverall=α×lossSSIM+(1−α)×lossL_1
% The layer calculates a multiscale structural similarity (SSIM) loss for the grayscale representations of the predicted and target RGB images using the multissim function.
...
% Convert the original underexposed RAW image to an RGB image in one step using the raw2rgb function.
...
% Convert the original well exposed RAW image to an RGB image in one step using the raw2rgb function.
% Display the result.
...
% Display the network prediction.
% The trained network recovers an impressive image under challenging acquisition conditions with very little noise or other visual artifacts.
