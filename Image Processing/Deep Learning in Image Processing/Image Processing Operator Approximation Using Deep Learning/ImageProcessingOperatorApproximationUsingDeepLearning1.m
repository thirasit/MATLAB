...
% For example, the multiscale context aggregation network (CAN) presented by Q. Chen [1] can approximate multiscale tone mapping, photographic style transfer, nonlocal dehazing, and pencil drawing.
...
% Create a randomPatchExtractionDatastore from the two image datastores.
...
% Two custom scale layers are added to implement an adaptive batch normalization layer.
% These layers are attached as supporting files to this example.
...
% Specify the hyperparameter settings by using the trainingOptions (Deep Learning Toolbox) function.
% Use the default values of 0.9 for "Momentum" and 0.0001 for "L2Regularization" (weight decay).
% Specify a constant learning rate of 0.0001.
...
% Train the multiscale CAN using the trainNetwork (Deep Learning Toolbox) function.
...
% To process an image using a trained multiscale CAN network that approximates a bilateral filter, follow the remaining steps of this example.
% The remainder of the example shows how to:
...
% Use the imbilatfilt function to apply a bilateral filter to the noisy image.
...
% Pass the normalized input image through the trained network and observe the activations (Deep Learning Toolbox) from the final layer (a regression layer).
